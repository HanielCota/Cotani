...
/**
 * Default implementation of {@link TabListController}.
 */
@InternalApi
public final class DefaultTabListController implements TabListController {
    ...
    public DefaultTabListController(PaperTaskScheduler scheduler) {
    ...
    private void render(Player player, PlayerTabListState state) {
        scheduler.entity(player, () -> {
            if (player.isOnline()) {
                renderNow(player, state);
            }
        });
    }

    /**
     * Renders the current header/footer values. Must be called from the owning entity thread.
     */
    private void renderNow(Player player, PlayerTabListState state) {
        ...
        synchronized void clearHeaderSubscription() {
            if (headerSubscription != null) {
                try {
                    ...
                    java.util.logging.Logger.getLogger(DefaultTabListController.class.getName())
